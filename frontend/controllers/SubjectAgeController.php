...
class SubjectAgeController extends Controller
{
    /**
    ...
    {
        $qB = SubjectAge::getActiveListQuery();
        $itemsPerPage = 8;
    ...
    public function actionView($id, $webpage)
    {
        ...
        return $this->render('view', [
            ...
            'webpage' => $webpage,
            'h1' => $subject->name,
            'subjectsWebpage' => Webpage::findOne(['module_id' => Module::getModuleIdByControllerAndAction('subject-age', 'index')]),
        ]);
    }
    ...
    {
        return $this->asJson(SubjectAge::getActiveListQuery()->asArray(true)->select(['id', 'name'])->all());
    }
    ...
    {
        if (($model = SubjectAge::findOne($id)) !== null && $model->active == SubjectAge::STATUS_ACTIVE) {
            return $model;
        } else {
